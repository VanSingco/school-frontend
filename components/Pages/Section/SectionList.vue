    ...
    <NuxtLayout :name="accessType">
        <div>
            <Header :breadcrumbs="header.breadcrumbs" :title="header.title">
                <nuxt-link :to="`/${props.accessType}/sections/add`" class="group relative flex justify-center rounded-md border border-transparent bg-indigo-600 py-3 px-4 text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
...
<script setup lang="ts">
    ...
    const columns = ['Name', 'Adviser', 'Grade Level', 'Student Count', 'Actions']
    ...
    function fetchSections() {
        ...
        section.list({search: search.value, orderBy: orderBy.value, page: page.value, paginate: true, school_id: null}).then((res) => {
